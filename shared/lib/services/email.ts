// Serviço de Email - Compatível com Node.js e Cloudflare Workers
import type { EnvConfig, EmailResult } from '../runtime/types';
...
export async function sendWelcomeEmail(env: EnvConfig, { to, funcionarioNome, empresaNome, loginUrl = 'https://app.livegenda.com' }: SendWelcomeEmailParams): Promise<EmailResult> {
  if (!env.RESEND_API) {
    console.error('[Email Boas-Vindas] RESEND_API não configurada');
    return { success: false, error: 'RESEND_API não configurada' };
  }
...
export async function sendAgendamentoConfirmacaoEmail(env: EnvConfig, params: SendAgendamentoConfirmacaoEmailParams): Promise<EmailResult> {
  if (!env.RESEND_API) {
    console.error('[Email Confirmação] RESEND_API não configurada');
    return { success: false, error: 'RESEND_API não configurada' };
  }
...
export async function sendAgendamentoRemarcacaoEmail(env: EnvConfig, params: SendAgendamentoRemarcacaoEmailParams): Promise<EmailResult> {
  if (!env.RESEND_API) {
    console.error('[Email Remarcação] RESEND_API não configurada');
    return { success: false, error: 'RESEND_API não configurada' };
  }
...
export async function sendAgendamentoCancelamentoEmail(env: EnvConfig, params: SendAgendamentoCancelamentoEmailParams): Promise<EmailResult> {
  if (!env.RESEND_API) {
    console.error('[Email Cancelamento] RESEND_API não configurada');
    return { success: false, error: 'RESEND_API não configurada' };
  }

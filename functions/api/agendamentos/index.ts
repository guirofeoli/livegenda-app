...
function validateAgendamento(body: any): { valid: boolean; error?: string } {
  ...
  if (!body.cliente_id || typeof body.cliente_id !== 'number') {
    return { valid: false, error: 'cliente_id é obrigatório' };
  }
  if (!body.funcionario_id || typeof body.funcionario_id !== 'number') {
    return { valid: false, error: 'funcionario_id é obrigatório' };
  }
  if (!body.servico_id || typeof body.servico_id !== 'number') {
...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  try {
    // Query base - filtros aplicados dinamicamente
    const empresaIdInt = parseInt(empresaId);
    
    let agendamentos;
    
    // Query simplificada (SQL dinâmico é complexo no tagged template)
    agendamentos = await db`
      SELECT 
        a.*,
        json_build_object('id', c.id, 'nome', c.nome, 'telefone', c.telefone) as cliente,
        json_build_object('id', f.id, 'nome', f.nome, 'cor_agenda', f.cor_agenda) as funcionario,
      ...
      WHERE a.empresa_id = ${empresaIdInt}
    ...
    if (funcionarioId) {
      agendamentos = agendamentos.filter((a: any) => a.funcionario_id === parseInt(funcionarioId));
    }
    if (clienteId) {
      agendamentos = agendamentos.filter((a: any) => a.cliente_id === parseInt(clienteId));
    }
    if (status) {
  ...
  try {
    ...
    const [agendamento] = await db`
      INSERT INTO agendamentos (
        ...
        data_hora, data_hora_fim, status, valor, observacoes, origem
      ) VALUES (
        ${body.empresa_id}, ${body.cliente_id}, ${body.funcionario_id},
        ${body.servico_id}, ${dataHoraInicio.toISOString()}, 
        ${dataHoraFim.toISOString()}, ${body.status || 'agendado'},
        ${body.valor || servicoInfo[0].preco || null}, ${body.observacoes || null},
        ${body.origem || 'manual'}
      )
      RETURNING id, data_hora, data_hora_fim, status, created_at
    `;

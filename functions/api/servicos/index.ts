...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  try {
    const servicos = await sql`SELECT * FROM servicos WHERE empresa_id = ${empresaId} AND ativo = true ORDER BY nome`;
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    const { empresa_id, nome, descricao, duracao_minutos, preco } = body;
    ...
    const result = await sql`INSERT INTO servicos (empresa_id, nome, descricao, duracao_minutos, preco, ativo) VALUES (${empresa_id}, ${nome}, ${descricao || null}, ${duracao_minutos || 30}, ${preco || 0}, true) RETURNING *`;
